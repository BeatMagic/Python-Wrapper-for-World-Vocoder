...
from os.path import join, exists, abspath, basename, normpath, dirname
...
class build_ext(_build_ext):
    ...
    def _world_commit(self):
        """Return the short git SHA of lib/World, or 'unknown'."""
        try:
            out = subprocess.check_output(
                ['git', '-C', WORLD_LIB_TOP, 'rev-parse', '--short', 'HEAD'],
    ...
    def _write_build_info(self, backend, avx512_on):
        ...
        ts = datetime.datetime.now(datetime.timezone.utc).strftime(
            '%Y-%m-%dT%H:%M:%SZ')
        content = (
            '"""Auto-generated by setup.py; do not edit by hand."""\n'
            'FFT_BACKEND = {fft!r}\n'
            'AVX512_ENABLED = {avx!r}\n'
            'WORLD_COMMIT = {commit!r}\n'
            'BUILD_TIMESTAMP = {ts!r}\n'
        ).format(
            fft=backend,
            avx=bool(avx512_on),
    ...
    def build_extensions(self):
        ...
        # Step 5b: write pyworld/_build_info.py so the final wheel carries
        # runtime-queryable metadata. Lets users verify their install with
        # `pyworld.__fft_backend__` / `pyworld.build_info()` even when pip
        # ran without -v and the [pyworld] prints were captured.
        self._write_build_info(backend, avx512_ok)
